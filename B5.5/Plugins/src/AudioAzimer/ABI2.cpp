...
void MIXER2 () { // Needs accuracy verification...
	...
	for (int x=0; x < (int)count; x+=2) { // I think I can do this a lot easier 
...
void ADDMIXER () {
	...
	u16 InBuffer  = (u16)(t9 >> 16);
	u16 OutBuffer = (u16)(t9 & 0xffff);
...
void HILOGAIN () {
	u16 cnt = (u16)(k0 & 0xffff);
	u16 out = (u16)((t9 >> 16) & 0xffff);
	s16 hi  = (s16)((k0 >> 4) & 0xf000);
	u16 lo  = (u16)((k0 >> 20) & 0xf);
	s16 *src;
