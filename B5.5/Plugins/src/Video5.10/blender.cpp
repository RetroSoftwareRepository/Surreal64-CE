...
{
	//1. Z_CMP        -- Enable / Disable Zbuffer compare, SetRenderState( D3DRS_ZENABLE )
	//	1   -   Enable ZBuffer
	//	0   -   Disable ZBuffer

	//2. Z_UPD        -- Enable / Disable Zbuffer update, SetRenderState( D3DRS_ZWRITEENABLE )
	...
	//4.  ZMode       
	//	#define	ZMODE_OPA	0           -- Usually used with Z_CMP and Z_UPD
	//											   or used without neither Z_CMP or Z_UPD
	//											   if used with Z_CMP and Z_UPD, then this is
	//											   the regular ZBuffer mode, with compare and update
	//	#define	ZMODE_INTER	0x400
	//	#define	ZMODE_XLU	0x800       -- Usually used with Z_CMP, but not with Z_UPD
	//											   Do only compare, no zbuffer update.
	//											   Not output if the z value is the same
	//	#define	ZMODE_DEC	0xc00       -- Usually used with Z_CMP, but not with Z_UPD
	...
	CDaedalusRender *render = CDaedalusRender::g_pRender;
...
#define BLEND_XLU				0x0040
	...
	u32 blendmode_1 = u32( gRDP.otherMode.blender & 0xcccc );
	u32 blendmode_2 = u32( gRDP.otherMode.blender & 0x3333 );
	u32 cycletype = gRDP.otherMode.cycle_type;
	...
	case CYCTYPE_2CYCLE:
		...
		*/
		...
		default:
			...
			if( pauseAtNext )
			{
				u32 dwM1A_1 = (gRDP.otherMode.blender>>14) & 0x3;
				u32 dwM1B_1 = (gRDP.otherMode.blender>>10) & 0x3;
				u32 dwM2A_1 = (gRDP.otherMode.blender>>6) & 0x3;
				u32 dwM2B_1 = (gRDP.otherMode.blender>>2) & 0x3;

				u32 dwM1A_2 = (gRDP.otherMode.blender>>12) & 0x3;
				u32 dwM1B_2 = (gRDP.otherMode.blender>>8) & 0x3;
				u32 dwM2A_2 = (gRDP.otherMode.blender>>4) & 0x3;
				u32 dwM2B_2 = (gRDP.otherMode.blender   ) & 0x3;
