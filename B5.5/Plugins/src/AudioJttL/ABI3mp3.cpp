...
*/
				...
				*(s16 *)(mp3data+((t0+(short)0xFFA0))) = (short)v[2];
				...
				for (int x = 0; x < 8; x++) {
					...
					//Clamp(v18);
					// clamp???
					*(s16 *)(mp3data+(outPtr^2)) = (short)v0;
					*(s16 *)(mp3data+((outPtr+2)^2)) = (short)v18;
					outPtr+=4;
					addptr += 0x30;
					offset += 0x38;
				}
				...
				if (t4 & 0x2) {
					...
					*(s16 *)(mp3data+(outPtr^2)) = (short)v2;
				} else {
					v4 = (v4 * *(u32 *)(mp3data+0xCE8)) >> 0x10;
					*(s16 *)(mp3data+(outPtr^2)) = (short)v4;
				...
				for (x = 0; x < 8; x++) {
					...
					//Clamp(v18);
					// clamp???
					*(s16 *)(mp3data+((outPtr+2)^2)) = (short)v0;
					*(s16 *)(mp3data+((outPtr+4)^2)) = (short)v18;
					outPtr+=4;
					addptr -= 0x50;
				}
				...
				for (i = 0; i < 8; i++) {
					...
					*(s16 *)((u8 *)mp3data+((tmp-0x40)^2)) = (s16)v;
					...
					if (v > 32767) v = 32767; else if (v < -32767) v = -32767;
					*(s16 *)((u8 *)mp3data+((tmp-0x30)^2)) = (short)v;
					// v2
					v = (*(s16 *)(mp3data+((tmp-0x1E)^2)) * hi1);
					if (v > 32767) v = 32767; else if (v < -32767) v = -32767;
					*(s16 *)((u8 *)mp3data+((tmp-0x1E)^2)) = (short)v;
					// v4
					v = (*(s16 *)(mp3data+((tmp-0xE)^2)) * hi1);
					if (v > 32767) v = 32767; else if (v < -32767) v = -32767;
					*(s16 *)((u8 *)mp3data+((tmp-0xE)^2)) = (short)v;
					tmp += 2;
				}
}