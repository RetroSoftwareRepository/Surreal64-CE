...
CTexture::CTexture(DWORD dwWidth, DWORD dwHeight, TextureUsage usage) :
	m_pTexture(NULL),
...
void CTexture::ScaleImageToSurface(bool scaleS, bool scaleT)
{
	...
	DWORD width = scaleS ? m_dwWidth : m_dwCreatedTextureWidth;
	DWORD height = scaleT ? m_dwHeight : m_dwCreatedTextureHeight;
...
void CTexture::RestoreAlphaChannel(void)
{
	...
	if ( StartUpdate(&di) )
	{
		...
		for (DWORD y = 0; y < m_dwHeight; y++)
		{
			...
			{
				DWORD dw = dwSrc[x];
				DWORD dwRed   = (BYTE)((dw & 0x00FF0000)>>16);
				DWORD dwGreen = (BYTE)((dw & 0x0000FF00)>>8 );
				DWORD dwBlue  = (BYTE)((dw & 0x000000FF)    );
				DWORD dwAlpha = (dwRed+dwGreen+dwBlue)/3;
				dw &= 0x00FFFFFF;
				dw |= (dwAlpha<<24);
