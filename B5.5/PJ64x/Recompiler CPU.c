...
BYTE * Compiler4300iBlock(void) {
	...
	BlockInfo.CompiledLocation = XBOXRecompPos;
	...
	for (count = 0; count < BlockInfo.ExitCount; count ++) {
		...
		SetJump32(BlockInfo.ExitInfo[count]->JumpLoc,XBOXRecompPos);	
	...
	if (ShowCompMem) {
		...
		Size = XBOXRecompPos - XBOXRecompCode;
		MB = Size / 0x100000;
		Size -= MB * 0x100000;
		KB = Size / 1024;
		Size -= KB  * 1024;
		sprintf(StatusString,"Memory used: %d mb %d kb %d bytes\n ",MB,KB,Size);
		//OutputDebugString(StatusString);
...
void CompileExit (DWORD TargetPC, REG_INFO ExitRegSet, int reason, int CompileNow, void (*x86Jmp)(char * Label, DWORD Value)) {
	...
	if (!CompileNow) {
		...
		BlockInfo.ExitInfo[BlockInfo.ExitCount]->JumpLoc = XBOXRecompPos - 4;
		...
		if (SelfModCheck == ModCode_ChangeMemory) {
			...
			*((BYTE *)(Jump))=(BYTE)(XBOXRecompPos - Jump - 1);
			if (Jump2 != NULL) {
				...
				*((BYTE *)(Jump2))=(BYTE)(XBOXRecompPos - Jump2 - 1);
			}
			...
			if (TargetPC >= 0x80000000 && TargetPC < 0xC0000000) {
				...
				MoveVariableToX86reg((BYTE *)XBOXJumpTable + pAddr,"XBOXJumpTable + pAddr",x86_ECX);
				Jump2 = NULL;
			} else {				
				MoveConstToX86reg((TargetPC >> 12),x86_ECX);
				MoveConstToX86reg(TargetPC,x86_EBX);
				...
				JeLabel8("NoTlbEntry",0);
				Jump2 = XBOXRecompPos - 1;
				AddConstToX86Reg(x86_ECX,(DWORD)XBOXJumpTable - (DWORD)N64MEM);
	...
	if ((Section->CompilePC & 0xFFC) == 0xFFC) {
		...
		for (count = 0; count < 2; count ++) {
			...
			if (JumpInfo[count]->LinkLocation != NULL) {
				SetJump32(JumpInfo[count]->LinkLocation,XBOXRecompPos);
				JumpInfo[count]->LinkLocation = NULL;
				if (JumpInfo[count]->LinkLocation2 != NULL) { 
					SetJump32(JumpInfo[count]->LinkLocation2,XBOXRecompPos);
...
//CPU_Message("PermLoop ***");
			...
			JmpLabel32(Label,0);
			SetJump32((DWORD *)XBOXRecompPos - 1,TargetSection[count]->CompiledLocation);
		}
	}
	...
	for (count = 0; count < 2; count ++) {
		...
		for (count2 = 0;TargetSection[count]->ParentSection[count2] != NULL;count2++) {
			...
			if (JumpInfo[count]->FallThrough) { 
				...
				JumpInfo[count]->LinkLocation = XBOXRecompPos - 4;
			}
		}
	}
...
BOOL GenerateX86Code (BLOCK_SECTION * Section, DWORD Test) {
	...
	Section->CompiledLocation = XBOXRecompPos;
	Section->CompilePC = Section->StartPC;
	NextInstruction = NORMAL;	
	/*if (CPU_Type == CPU_SyncCores) { 
	//if (CPU_Type == CPU_SyncCores && (DWORD)XBOXRecompPos > 0x6094C283) { 
		...
		MoveConstToVariable((DWORD)XBOXRecompPos,&CurrentBlock,"CurrentBlock");
	}*/
	do {
		__try {
		...
		/*if ((DWORD)XBOXRecompPos > 0x60B452E6) {
		...
		/*if ((DWORD)XBOXRecompPos > 0x60AD0BD3) {
...
BOOL InheritParentInfo (BLOCK_SECTION * Section) {
	...
	if (NoOfParents == 0) { 
		...
		if (JumpInfo->LinkLocation != NULL) {
			...
			SetJump32(JumpInfo->LinkLocation,XBOXRecompPos);
			if (JumpInfo->LinkLocation2 != NULL) { 
				SetJump32(JumpInfo->LinkLocation2,XBOXRecompPos);
			}
		}
		memcpy(&Section->RegWorking,&Section->RegStart,sizeof(REG_INFO));
		return TRUE;
	...
	for (count = 0;count < NoOfCompiledParents;count++) {
		...
		JmpLabel32(Label,0);		
		JumpInfo->LinkLocation  = XBOXRecompPos - 4;
		JumpInfo->LinkLocation2 = NULL;
...
void StartRecompilerCPU (void ) { 
	...
	__try {
		if (SelfModCheck == ModCode_ChangeMemory) {
			...
			for (;;) {
				...
				if (NextInstruction == DELAY_SLOT) {
					__try {
						Value = (DWORD)(*(XBOXDelaySlotTable + (Addr >> 12)));
					...
					if (Block == NULL) {
						...
						TargetIndex += 1;
						*(XBOXDelaySlotTable + (Addr >> 12)) = (void *)Value;
						NextInstruction = NORMAL;
					}
					_asm {
						pushad
		...
		for (;;) {
			...
			if (NextInstruction == DELAY_SLOT) {
				__try {
					Block = *(XBOXDelaySlotTable + (Addr >> 12));
				} __except(EXCEPTION_EXECUTE_HANDLER) {
					DisplayError("Executing Delay Slot from non maped space");
					ExitThread(0);
				}
				if (Block == NULL) {
					...
					Block = CompileDelaySlot();
					*(XBOXDelaySlotTable + (Addr >> 12)) = Block;
			...
			if (Block == NULL) {
				...
				__try {
					...
					//DisplayError("Reset Recompiler Code %X",XBOXRecompPos - RecompCode);
					ResetRecompCode();
					Block = Compiler4300iBlock();
				}
				if (SelfModCheck == ModCode_CheckMemoryCache) {
					...
					*(XBOXJumpTable + (Addr >> 2)) = &TargetInfo[TargetIndex];
					TargetIndex += 1;
