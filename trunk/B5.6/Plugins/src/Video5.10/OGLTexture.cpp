...
COGLTexture::COGLTexture(DWORD dwWidth, DWORD dwHeight, bool asRenderTarget) :
	...
	m_glFmt(GL_RGBA)
{
...
#ifdef _DEBUG
	///if( (dwWidth != m_dwCreatedTextureWidth && m_dwCreatedTextureWidth/2+1 == dwWidth) ||
	//	(dwHeight != m_dwCreatedTextureHeight && m_dwCreatedTextureHeight/2+1 == dwHeight) )
	//{
	//	DebuggerAppendMsg("Texture: Width=%d, Height=%d", dwWidth, dwHeight);
	//}
#endif
	...
	if( options.OpenglColorBufferSetting == TEXTURE_FMT_A4R4G4B4 ) m_glFmt = GL_RGBA4;
	DEBUGGER_ONLY_IF( logTextureDetails, {DebuggerAppendMsg("New texture: (%d, %d)", dwWidth, dwHeight);});
}
