...
__inline static int16_t clamp_s16(int_fast32_t x)
{
...
__inline static unsigned int align(unsigned int x, unsigned amount)
{
    --amount;
...
__inline static uint8_t* const dmem_u8(uint16_t address)
{
    return (uint8_t*)(&rsp.DMEM[(address & 0xfff) ^ S8]);
}

__inline static uint16_t* const dmem_u16(uint16_t address)
{
    assert((address & 1) == 0);
    return (uint16_t*)(&rsp.DMEM[(address & 0xfff) ^ S16]);
}

__inline static uint32_t* const dmem_u32(uint16_t address)
{
    assert((address & 3) == 0);
    return (uint32_t*)(&rsp.DMEM[(address & 0xfff)]);
}

__inline static uint8_t* const dram_u8(uint32_t address)
{
    return (uint8_t*)&rsp.RDRAM[(address & 0xffffff) ^ S8];
}

__inline static uint16_t* const dram_u16(uint32_t address)
{
    assert((address & 1) == 0);
    return (uint16_t*)&rsp.RDRAM[(address & 0xffffff) ^ S16];
}

__inline static uint32_t* const dram_u32(uint32_t address)
{
    assert((address & 3) == 0);
