...
typedef unsigned int (*adpcm_predict_frame_t)(short* dst, unsigned short dmemi, unsigned char scale);
...
void alist_adpcm(
        ...
        unsigned short dmemo,
        unsigned short dmemi,
        unsigned short count,
        const short* codebook,
        uint32_t loop_address,
        uint32_t last_frame_address)
{
    //assert((count & 0x1f) == 0);

    __int16 last_frame[16];
    int j;
    ...
    for(j = 0; j < 16; ++j, dmemo += 2)
        *(int16_t*)(BufferSpace + (dmemo ^ S16)) = last_frame[j];
	...
	while (count != 0) {
		...
		if(two_bit_per_sample)
			dmemi += adpcm_predict_frame_2bits(frame, dmemi, scale);
		else
			dmemi += adpcm_predict_frame_4bits(frame, dmemi, scale);
