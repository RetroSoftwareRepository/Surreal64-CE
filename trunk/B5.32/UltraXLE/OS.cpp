...
void os_switchcheck(void)
{
    ...
    if(t!=st.thread)
    {
        if(t==0)
        {
            xception("At %08X, no runnable threads!\n",st.pc);
                    ...
                    dword m_param,dword m_stack,dword priority)
{
    ...
    if(id<=0 || id>=MAXTHREAD || thread[id].active)
    {
        ...
        if(id==a) xception("Out of threads!");
    }
    if(id>=MAXTHREAD)
    {
        xception("thread id too large");
        return;
    }
...
void  osCreateMesgQueue(dword m_queue,dword m_mesg,dword size)
{
    ...
    if(!id || id>=MAXQUEUE)
    {
        xception("queue id too large");
        return;
    }
    if(size>MAXQUEUESIZE)
    {
        xception("queue size too large");
        return;
    }
...
dword osSendMesg(dword m_queue,dword m_mesg,int block)
{
    ...
    if(!id || id>=queuenum)
    {
        ...
        {
            xception("queue id %i at %08X too large",id,m_queue);
        }
        return(0);
    }
    if(os_queuecheck(id))
...
//            xception("queue %i full on OS messge send",id);
            return(-1);
        }
        if(!block) return(-1);
        // force a taskswitch
...
//    if(!block) xception("nonblocking RecvMesg! CHECK RETURN\n");
...
*/
    ...
    {
        xception("DmaCopy cartaddr %08X, changed to %08X, transfer done.\n",
            orgdevaddr,devaddr);
    }
    ...
    if(devaddr>cart.size || vaddr>8*1024*1024 || nbytes>cart.size-devaddr)
    {
        ...
        xception("DmaCopy illegal ranges");
        return(1);
    }

    if(direction!=0)
    {
        xception("DmaCopy write/other");
        return(1);
    }

    if(nbytes<0 || nbytes>16*1024*1024)
    {
        xception("DmaCopy overflow");
        return(1);
    }
    ...
    if(vaddr&3)
    {
        xception("DmaCopy nonaligned destination");
        return(1);
    }

    if(devaddr&1)
    {
        xception("DmaCopy nonaligned source");
        return(1);
    }
    ...
    {
        mem_writerangeraw(vaddr,nbytes,(char *)(cart.data+devaddr));
    }
...
int osEPiStartDma(dword m_pihandle,dword m_iomesg,dword flag)
{
    ...
    else
    {
        logo(BLUE"UNKNOWN TYPE (dram=%08X cart=%08X count=%08X)\n",
            dram,cart,count);
        xception("unsupported EPiStartDMA message\n");
    }
    return(0);
}
...
{ // hacks, called when we would switch to idle thread
    ...
    if(1)
    {
        ...
        if(st.thread==lastthread && abs((int)(st.pc-lastpc))<16)
        {
            loopcnt++;
            if(loopcnt>10000)
            {
