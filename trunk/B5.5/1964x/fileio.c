...
void FileIO_gzLoadStateFile(const char *filename)
{
	...
	{
		UnprotectAllBlocks();
		#ifdef REDUCE_RDRAM_COPY
		if (currentromoptions.RDRAM_Size != RDRAMSIZE_8MB)
			memset(RDRAM_Copy, 0xEE, MEMORY_SIZE_NO_EXPANSION);
		else
		#endif
			memset(RDRAM_Copy, 0xEE, MEMORY_SIZE_WITH_EXPANSION);
	}
