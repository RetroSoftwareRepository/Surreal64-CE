    ...
    if(XLMenu_CurMenu || XLMenu_CurRoutine)
    {
        if(gamepad->bPressedAnalogButtons[XINPUT_GAMEPAD_A] ||
           ...
           gamepad->wPressedButtons&XINPUT_GAMEPAD_BACK)
            command = MENU_BACK;
        else if((gamepad->fY1>0.25f) || (gamepad->wPressedButtons & XINPUT_GAMEPAD_DPAD_UP))
            command = MENU_PREVITEM;
        else if((gamepad->fY1<-0.25f) || (gamepad->wPressedButtons & XINPUT_GAMEPAD_DPAD_DOWN))
...
DWORD XLMenu_Routine(DWORD command)
{
    ...
    // get height of font
    itemh = (float)XLMenu_Font->GetFontHeight();
    ...
    for(i=firstitem; i<nitems+firstitem; i++)
    {
        ...
        itemy += XLMenu_Font->GetFontHeight();
    }
