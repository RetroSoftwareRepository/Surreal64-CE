...
class CProfiler : public CUnique< CProfiler >  
{
	...
	public:
		...
		void Reset()
		{
			...
			for ( DWORD i = 0; i < NUM_PROFILE_ITEMS; i++ )
			{
		...
		{
			QueryPerformanceCounter( &m_liStartTimes[ (DWORD)item ] );
		}

		void StopTiming( ProfileItem item );

		// Return the time spent on this item since resetting
		float GetTime( DWORD history, ProfileItem item )
		{
			...
			return m_fTimes[ actual_history ][ (DWORD)item ];
		}

		void Display();

		DWORD GetNumHistoryItems() const { return NUM_HISTORY_ITEMS; }
		DWORD GetNumProfileItems() const { return NUM_PROFILE_ITEMS; }

		const CHAR * GetProfileItemName( ProfileItem item )
		{
			return s_ItemInfo[ (DWORD) item ].name;
		}
