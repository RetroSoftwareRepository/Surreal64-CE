...
//	g_pD3DDev->SetRenderState( D3DRS_FOGVERTEXMODE, D3DFOG_LINEAR );
	...
	memset(&m_TSSInfo, 0, sizeof(TSSInfo)*4);
...
bool D3DRender::RenderTexRect()
{
	...
	{
		D3DVIEWPORT8 vp = {0, 0, g_FakeFrameBufferInfo.width, g_FakeFrameBufferInfo.height, 0, 1};
...
{
	m_fillRectVtx[0].y += toolbarHeightToUse;
	m_fillRectVtx[1].y += toolbarHeightToUse;
...
{
	m_line3DVector[0].y += toolbarHeightToUse;
	m_line3DVector[1].y += toolbarHeightToUse;
	m_line3DVector[2].y += toolbarHeightToUse;
	m_line3DVector[3].y += toolbarHeightToUse;
...
void D3DRender::UpdateScissor()
{
	if( options.enableHacks && g_CI.dwWidth == 0x200 && gRDP.scissor.right == 0x200 && g_CI.dwWidth>(*g_GraphicsInfo.VI_WIDTH_RG & 0xFFF) )
	{
		...
		D3DVIEWPORT8 vp = {0, toolbarHeightToUse, (DWORD)(width*windowSetting.fMultX), (DWORD)(height*windowSetting.fMultY), 0, 1};
		if( !gRSP.bNearClip )
			vp.MinZ = -10000;
		g_pD3DDev->SetViewport(&vp);
	}
	...
	else
	{
		D3DVIEWPORT8 vp = {
			(DWORD)(gRDP.scissor.left*windowSetting.fMultX), 
				(DWORD)(gRDP.scissor.top*windowSetting.fMultY+toolbarHeightToUse), 
...
void D3DRender::ApplyScissorWithClipRatio()
{
	D3DVIEWPORT8 vp = {
		(DWORD)(gRSP.real_clip_scissor_left*windowSetting.fMultX), 
			(DWORD)(gRSP.real_clip_scissor_top*windowSetting.fMultY)+toolbarHeightToUse, 
