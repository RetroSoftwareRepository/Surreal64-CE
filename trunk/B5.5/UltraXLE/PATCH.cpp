...
void p_dabort(void)
{
    xception("unimplemented dword routine");
}
...
{
	dword dwMQ         = A0.q;
	dword dwContStatus = A1.q;

	mem_write16(dwContStatus + 0, 0x8000);		// type
	mem_write8(dwContStatus + 2, 0);			// status
	mem_write8(dwContStatus + 3, 0);			// errno

	V0.q = 0;

	//MessageBox(NULL, "Reached status!", "YES", MB_OK);
}

void p_osEepromProbe()
{
	V0.q = 0x01;

	//MessageBox(NULL, "Reached probe!", "YES", MB_OK);
}

/*void p_osEepromRead()
{
	MessageBox(NULL, "Reached read!", "YES", MB_OK);
}

void p_osEepromLongRead()
{
	MessageBox(NULL, "Reached long read!", "YES", MB_OK);
}

void p_osEepromWrite()
{
	MessageBox(NULL, "Reached write!", "YES", MB_OK);
}

void p_osEepromLongWrite()
{
	MessageBox(NULL, "Reached long write!", "YES", MB_OK);
}*/
...
void op_patch(int patch)
{
    ...
    if(!patchtable[patch] || patch>=sizeof(patchtable)/4)
    {
        ...
        {
            st.branchdelay=0;
            xception("Execution at null page (st.pc=%08X).",st.pc);
        }
        else xception("Unimplemented patch %i!",patch);
        return;
    }
    patchtable[patch]();
}
