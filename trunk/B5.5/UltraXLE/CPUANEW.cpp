...
int vmcache_find(int reg,int off)
{
    ...
    {
        if(r.vmcache[i].reg==reg && abs((int)(r.vmcache[i].off-off))<120) return(i);
    }
    return(-1);
}
...
//    print("\n#cmp true %08X (%08X) false %08X (%08X)\n",truepc,jumptrue,falsepc,jumpfalse);
    ...
    else if(cmpop!=CMP_ALWAYS)
    {
        ...
        if(r.slt_branch)
        {
            ...
            {
                r.errors++;
                xception("invalid slt optimization (likely branch)\n");
            }
            ...
            {
                xception("invalid slt optimization\n");
                r.errors++;
            }
        }
    ...
    if(flags&BR_LIKELY)
    {
        ...
        default: xception("cpuanew: invalid ac_branch type %04X %04X\n",cmpop,flags);
        }
    ...
    else
    {
        ...
        else
        {
            ...
            {
                xception("cpuanew: branch EDX corrupted\n");
            }
...
void ac_fpulwsw(dword opcode,int op)
{
    ...
    */
    ...
    case 57: // SWC1
        ...
        break;
    default:
        xception("cpuanew: invalid fpulwsw op\n");
        break;
    }
}
...
//            i,r.pc,rs,imm,r.vmcache[i].reg,r.vmcache[i].off,addecx);
    ...
    else if(size>=0x10)
    {
        ...
        default: xception("cpuanew: illegal lwl-etc size\n"); break;
        }
    }
    else if(size==4)
    {
