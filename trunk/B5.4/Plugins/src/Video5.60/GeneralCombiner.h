...
public:
	...
	DWORD			dwTexture;	//Which texture to apply, 0 or 1
	bool			bTextureUsed;	
...
{
public:
	DWORD		muxDWords[4];	// Simplified Mux
	DWORD		dwMux0;
	DWORD		dwMux1;
	LONG		nStages;
	...
	DWORD		specularPostOp;
	DWORD		colorTextureFlag[2];		
...
protected:
	...
	int		GenCI_Type_A_MOD_C(int curN64Stage, int curStage, GeneralCombinerInfo &ci, DWORD dxop=CM_MODULATE);
	...
	int		LM_GenCI_Type_A_MOD_C(N64CombinerType &m, int curStage, int limit, int channel, bool checktexture, GeneralCombinerInfo &ci, DWORD dxop=CM_MODULATE);
	...
	bool	LM_Check1TxtrForAlpha(int curStage, GeneralCombinerInfo &ci, DWORD val);
