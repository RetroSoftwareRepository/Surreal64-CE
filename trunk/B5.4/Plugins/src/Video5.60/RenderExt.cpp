...
void CRender::LoadFrameBuffer(bool useVIreg, DWORD left, DWORD top, DWORD width, DWORD height)
{
	...
	if( useVIreg && *g_GraphicsInfo.VI_ORIGIN_REG > VIwidth*2 )
	{
		...
		gti.PalAddress = (DWORD)(&g_wRDPTlut[0]);
	...
	gti.tileNo = -1;
	TextureEntry *pEntry = gTextureManager.GetTexture(&gti, false, true, false);
	if( pEntry ) SetCurrentTexture( 0, pEntry->pTexture, pEntry->ti.WidthToCreate, pEntry->ti.HeightToCreate, pEntry);
}

void CRender::LoadTextureFromMemory(void *buf, DWORD left, DWORD top, DWORD width, DWORD height, DWORD pitch, DWORD format)
{
	TextureInfo gti;
	gti.Format	= g_CI.dwFormat;
	gti.Size	= g_CI.dwSize;
	...
	gti.tileNo = -1;
	TextureEntry *pEntry = gTextureManager.GetTexture(&gti, false);
...
void CRender::LoadObjBGCopy(uObjBg &info)
{
	...
	if( options.bEnableHacks )
	{
		...
		{
			// Hack for RE2
			DWORD w = *g_GraphicsInfo.VI_WIDTH_REG & 0xFFF;
...
void CRender::LoadSprite2D(Sprite2DInfo &info, DWORD ucode)
{
	TextureInfo gti;
	...
	gti.Palette	= 0;
	gti.PalAddress = (DWORD)(g_pRDRAMu8+RSPSegmentAddr(info.spritePtr->TlutPointer));

	if( options.enableHackForGames == HACK_FOR_NITRO )
	{
		gti.WidthToCreate	= DWORD(info.spritePtr->SubImageWidth/info.scaleX);
		gti.HeightToCreate	= DWORD(info.spritePtr->SubImageHeight/info.scaleY);
		gti.LeftToLoad		= DWORD(info.spritePtr->SourceImageOffsetS/info.scaleX);
		gti.TopToLoad		= DWORD(info.spritePtr->SourceImageOffsetT/info.scaleY);
		gti.Pitch	= info.spritePtr->Stride << gti.Size >> 1;
		gti.Pitch	= DWORD(gti.Pitch*info.scaleY);
	}
	else
	{
...
void CRender::DrawSprite2D(Sprite2DInfo &info, DWORD ucode)
{
...
void CRender::DrawSpriteR(uObjTxSprite &sprite, bool initCombiner, DWORD tile, DWORD left, DWORD top, DWORD width, DWORD height)	// With Rotation
{
	status.bCIBufferIsRendered = true;
	if( status.bHandleN64TextureBuffer )
	{
...
void CRender::DrawFrameBuffer(bool useVIreg, DWORD left, DWORD top, DWORD width, DWORD height)
{
...
{
	DWORD imageWidth = bg.imageW/4;
	DWORD imageHeight = bg.imageH/4;

	TextureInfo gti;
	gti.Format	= bg.imageFmt;
	gti.Size		= bg.imageSiz;

	BYTE* img = (BYTE*)(g_pRDRAMu8+RSPSegmentAddr(bg.imagePtr));
	
	DWORD palAddr = (DWORD)(&g_wRDPTlut[0]);
...
void CRender::LoadObjSprite(uObjTxSprite &sprite, bool useTIAddr)
{
	...
	if( useTIAddr )
	{
		img = (BYTE*)(g_pRDRAMu8+RSPSegmentAddr(g_TI.dwAddr));
	}
	else
		img = (BYTE*)(g_pRDRAMu8+RSPSegmentAddr(sprite.txtr.block.image));
	DWORD palAddr = (DWORD)(&g_wRDPTlut[0]);
