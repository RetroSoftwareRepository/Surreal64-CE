...
D3DCOLOR CalculateConstFactor(DWORD colorOp, DWORD alphaOp, DWORD curCol)
{
	N64CombinerType m;
	IColor color(curCol);
	IColor alpha(curCol);
	...
	*(DWORD*)&m = alphaOp;
	...
	return (D3DCOLOR)(((DWORD)color&0x00FFFFFF)|((DWORD)alpha&0xFF000000));
}


D3DCOLOR CColorCombiner::GetConstFactor(DWORD colorFlag, DWORD	alphaFlag, DWORD defaultColor)
{
...
{
	LOG_DL(cycleTypeStrs[gRDP.otherMode.cycle_type]);
