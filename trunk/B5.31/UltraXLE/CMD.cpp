...
void loadstate(char *name)
{
    ...
    if(f1)
    {
        ...
        else
        {
            ...
            st2.xception=0;
            mem_load(f1);
            if(!st2.xception) cpu_load(f1);
            if(!st2.xception) os_load(f1);
            if(!st2.xception) sym_addpatches();
            if(!st2.xception) print("State loaded from %s\n",name);
