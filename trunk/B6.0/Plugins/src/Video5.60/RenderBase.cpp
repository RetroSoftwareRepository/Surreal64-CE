...
void (*ProcessVertexData)(DWORD dwAddr, DWORD dwV0, DWORD dwNum)=NULL;
...
void InitVertex(DWORD dwV, DWORD vtxIndex, bool bTexture, bool openGL)
{
	...
	DEBUGGER_ONLY_IF( logVertex, 
	{
		DebuggerAppendMsg("Vertex: %d: (%f, %f, %f, %f), U=%f, V=%f\n  DIF(%08X), SPE(%08X)\n", dwV, g_vecProjected[dwV].x, g_vecProjected[dwV].y, g_vecProjected[dwV].z, g_vecProjected[dwV].w,
			g_fVtxTxtCoords[dwV].x, g_fVtxTxtCoords[dwV].y, v.dcDiffuse, v.dcSpecular );
...
inline void ReplaceAlphaWithFogFactor(int i)
{
	if( gRDP.geometryMode & G_FOG )
	{
		...
		if( g_vecProjected[i].z > 1 )
			*(((BYTE*)&(g_dwVtxDifColor[i]))+3) = 0xFF;
		if( g_vecProjected[i].z < 0 )
			*(((BYTE*)&(g_dwVtxDifColor[i]))+3) = 0;
		else
			*(((BYTE*)&(g_dwVtxDifColor[i]))+3) = (BYTE)(g_vecProjected[i].z*255);	
	}
}
...
step3:
		...
		if( gRSP.bLightingEnable )
		{
			...
			*(((BYTE*)&(g_dwVtxDifColor[i]))+3) = vert.rgba.a;	// still use alpha from the vertex
		}
		else
		{
	...
	case RSP_MV_WORD_OFFSET_POINT_RGBA:		// Modify RGBA
		{
			...
			LOG_DL("Modify vert %d color, 0x%08x", vertex, g_dwVtxDifColor[vertex]);
		}
		break;
	case RSP_MV_WORD_OFFSET_POINT_XYSCREEN:		// Modify X,Y
		{
			...
			LOG_DL("Modify vert %d: x=%d, y=%d", vertex, x, y);
			DEBUGGER_IF_DUMP( logVertex, {DebuggerAppendMsg("Modify vert %d: (%d,%d)", vertex, x, y);});
		}
		break;
	case RSP_MV_WORD_OFFSET_POINT_ZSCREEN:		// Modify C
		{
			...
			LOG_DL("Modify vert %d: z=%d", vertex, z);
			DEBUGGER_IF_DUMP( logVertex, {DebuggerAppendMsg("Modify vert %d: z=%d", vertex, z);});
		}
		break;
	case RSP_MV_WORD_OFFSET_POINT_ST:		// Texture
		{
			...
			float ftu = tu / 32.0f;
			float ftv = tv / 32.0f;
			LOG_DL("      Setting vertex %d tu/tv to %f, %f", vertex, (float)tu, (float)tv);
...
void ProcessVertexDataDKR(DWORD dwAddr, DWORD dwV0, DWORD dwNum)
{
	...
	DWORD pVtxBase = DWORD(g_pRDRAMu8 + dwAddr);
	...
	LOG_DL("    ProcessVertexDataDKR, CMatrix = %d, Add base=%s", gRSP.DKRCMatrixIndex, gRSP.DKRVtxAddBase?"true":"false");
#ifdef _DEBUG
	if( pauseAtNext && logVertex ) 
	{
...
void ProcessVertexDataPD(uint32 dwAddr, uint32 dwV0, uint32 dwNum)
{
	...
	for (DWORD i = dwV0; i < dwV0 + dwNum; i++)
	{
		...
		BYTE *addr = g_pRDRAMu8+dwPDCIAddr+ (vert.cidx&0xFF);
		DWORD a = addr[0];
		DWORD r = addr[3];
		DWORD g = addr[2];
		DWORD b = addr[1];
		...
		if( gRSP.bLightingEnable )
		{
			...
			*(((BYTE*)&(g_dwVtxDifColor[i]))+3) = (BYTE)a;	// still use alpha from the vertex
		}
		else
		{
			if( (gRDP.geometryMode & G_SHADE) == 0 && gRSP.ucode < 5 )	//Shade is disabled
...
void ProcessVertexData_Rogue_Squadron(uint32 dwXYZAddr, uint32 dwColorAddr, uint32 dwXYZCmd, uint32 dwColorCmd)
{
	...
	DWORD dwV0 = 0;
	DWORD dwNum = (dwXYZCmd&0xFF00)>>10;
	...
	for (i = dwV0; i < dwV0 + dwNum; i++)
	{
		...
		if( gRSP.bLightingEnable )
		{
			...
			*(((BYTE*)&(g_dwVtxDifColor[i]))+3) = vertcolors.a;	// still use alpha from the vertex
		}
		else
		{
			if( (gRDP.geometryMode & G_SHADE) == 0 && gRSP.ucode < 5 )	//Shade is disabled
