...
int CDirectXPixelShaderCombiner::GeneratePixelShaderFromMux(void)
{
	...
	HRESULT e = XGAssembleShader(NULL, m_textBuf, strlen(m_textBuf), SASMT_PIXELSHADER | /*SASM_OPTIMIZE_PIXELSHADER*/1, NULL, &pCompiledShader, NULL, NULL, NULL, NULL, NULL);
	if( e != S_OK )
	{
#ifdef _DEBUG
	...
	e = g_pD3DDev->CreatePixelShader( newEntry.pVS, &(newEntry.dwShaderID) );
	if( e != S_OK )
	{
...
void CDirectXPixelShaderCombiner::InitCombinerCycle12(void)
{
	...
	g_pD3DDev->SetPixelShader(dwShaderID);

	// Step 2: set constant colors
	float *pf;
	pf = GetPrimitiveColorfv();
	g_pD3DDev->SetPixelShaderConstant(2,pf,1);

	pf = GetEnvColorfv();
	g_pD3DDev->SetPixelShaderConstant(3,pf,1);

	float frac = gRDP.LODFrac / 255.0f;
	float tempf[4] = {frac,frac,frac,frac};
	g_pD3DDev->SetPixelShaderConstant(4,tempf,1);

	float frac2 = gRDP.primLODFrac / 255.0f;
	float tempf2[4] = {frac2,frac2,frac2,frac2};
	g_pD3DDev->SetPixelShaderConstant(5,tempf2,1);
...
void CDirectXSemiPixelShaderCombiner::InitCombinerCycle12(void)
{
	...
	else
	{
		...
		m_pD3DRender->D3DSetTexCoordIndex( 0, 0 );
		m_pD3DRender->D3DSetTexCoordIndex( 1, 1 );
