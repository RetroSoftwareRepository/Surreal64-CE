...
void DLParser_Process(OSTask * pTask)
{
	...
	if (_INPUT_IsIngameMenuWaiting())
	{
		...
		try{
			//gTextureCache.RecycleAllTextures();
			gTextureCache.DropTextures();
			gTextureCache.PurgeOldTextures();
			RDP_Cleanup();
...
{ 
	...
	if (g_DI.dwAddr == g_CI.dwAddr)
	{
		...
		if( currentRomOptions.bEmulateClear )
		{
			// Emulating Clear, by write the memory in RDRAM
			WORD color = (WORD)gRDP.originalFillColor;
			u32 pitch = g_CI.dwWidth<<1;
			DWORD base = (DWORD)(g_pu8RamBase + g_CI.dwAddr);
	...
	else if( status.bHandleN64TextureBuffer )
	{
		...
		g_FakeFrameBufferInfo.maxUsedHeight = max(g_FakeFrameBufferInfo.maxUsedHeight,y1);

		if( status.bDirectWriteIntoRDRAM || ( x0==0 && y0==0 && (x1 == g_FakeFrameBufferInfo.width || x1 == g_FakeFrameBufferInfo.width-1 ) ) )
		{
		...
		if( !status.bDirectWriteIntoRDRAM )
		{
			...
			if( x0==0 && y0==0 && (x1 == g_FakeFrameBufferInfo.width || x1 == g_FakeFrameBufferInfo.width-1 ) && gRDP.fillColor == 0)
			{
				CDaedalusRender::g_pRender->ClearBuffer(true,false);
			}
			else
			{
	...
	else
	{
		...
		if( gRDP.otherMode.cycle_type == CYCTYPE_FILL )
		{
			if( !status.bHandleN64TextureBuffer || g_FakeFrameBufferInfo.CI_Info.dwSize == G_IM_SIZ_16b )
			{
				CDaedalusRender::g_pRender->FillRect(x0, y0, x1, y1, gRDP.fillColor);
			}
		}
