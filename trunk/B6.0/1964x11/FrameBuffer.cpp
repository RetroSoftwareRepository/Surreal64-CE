...
// Ez0n3 - Rice plugins conflict with RecentCIInfo/SetVIScales - rename should work on both xbox and win32?
RecentCIInfo1964 g_RecentCIInfo1964[3];
RecentCIInfo1964 *g_uRecentCIInfoPtrs1964[3] =
{
	&g_RecentCIInfo1964[0],
	&g_RecentCIInfo1964[1],
	&g_RecentCIInfo1964[2],
};
...
{
	memset(g_RecentCIInfo1964, 0, sizeof(g_RecentCIInfo1964));
	memset(&curFrameBufInfos, 0, sizeof(curFrameBufInfos));
}

void SetVIScales1964()
{
	float xscale, yscale;
...
void UpdateRecentFrmBufAddr()
{
	...
	temp = g_uRecentCIInfoPtrs1964[2];
	g_uRecentCIInfoPtrs1964[2] = g_uRecentCIInfoPtrs1964[1];
	g_uRecentCIInfoPtrs1964[1] = g_uRecentCIInfoPtrs1964[0];

	g_uRecentCIInfoPtrs1964[0] = temp;
	temp->dwAddr = addr;
...
int GetPtrIdex(uint32 addr)
{
	...
	{
		if( g_uRecentCIInfoPtrs1964[1]->dwAddr <= addr && addr < g_uRecentCIInfoPtrs1964[1]->dwHighAddr )
			return 1;
		else if( g_uRecentCIInfoPtrs1964[0]->dwAddr <= addr && addr < g_uRecentCIInfoPtrs1964[0]->dwHighAddr )
			return 0;
		else if( g_uRecentCIInfoPtrs1964[2]->dwAddr <= addr && addr < g_uRecentCIInfoPtrs1964[2]->dwHighAddr )
			return 2;
		else
			return -1;
	}
...
void SetFrameBufferHasBeenWritten(uint32 addr)
{
	...
	if( (i = GetPtrIdex(addr)) != -1 )
	{
		...
		{
			if( !emustatus.VideoPluginProvideFrameBufferInfo )
				g_uRecentCIInfoPtrs1964[i]->bHasBeenWritten = TRUE;
		}
	}
}
...
void ProtectFrameBufferMemory()
{
	...
	{
		RecentCIInfo1964 *info = g_uRecentCIInfoPtrs1964[0];
		...
		if( i == -1 )
		{
			addr = g_uRecentCIInfoPtrs1964[2]->dwAddr;
			if( addr != 0 || emustatus.FrameBufferProtectionNeedToBeUpdate == TRUE )
			{
				num = g_uRecentCIInfoPtrs1964[2]->dwMemSize/0x1000;
				TRACE1("Unprotect old frame buffer at: %08X", g_uRecentCIInfoPtrs1964[2]->dwAddr);
			...
			g_uRecentCIInfoPtrs1964[0]->bHasBeenRead = FALSE;
			g_uRecentCIInfoPtrs1964[0]->bHasBeenWritten = FALSE;
			g_uRecentCIInfoPtrs1964[0]->bCopied = FALSE;
			num = (g_uRecentCIInfoPtrs1964[0]->dwMemSize+0xfff)/0x1000;
			TRACE1("Protect new frame buffer at: %08X", g_uRecentCIInfoPtrs1964[0]->dwAddr);
		...
		{
			RecentCIInfo1964 *temp = g_uRecentCIInfoPtrs1964[i];
			if( i == 1 )
			{
				g_uRecentCIInfoPtrs1964[1] = g_uRecentCIInfoPtrs1964[0];
				g_uRecentCIInfoPtrs1964[0] = temp;
