...
BOOL ReadRomData(char *rompath)
{
	...
	if(fp != NULL)
	{
		...
		for(i = 0; i < gROMLength && To_Stop_Reading_Rom_File == FALSE; i += 65536)
		{
			...
			fread(gMS_ROM_Image + i, sizeof(uint8), 65536, fp);
			{
				...
				strcat(fname, ext);
				sprintf(generalmessage, "%s [%s] %d%%", "Loading", fname, i * 100 / gROMLength);
			}
	...
	{
		MessageBox(0, "File could not be opened.", "Error", 0);
		return FALSE;
	}
...
{
#ifdef _XBOX
	return TRUE; //surreal old returns true
...
#ifdef USE_ROM_PAGING //_XBOX - not a part of the paging - but needed to convert roms for use with it
//surreal - for rom to temp rom - done in launcher
	...
	switch (bswaptype)
	{
		...
		case 2:
		{
			...
			_asm
			{
				...
				// Ez0n3 - diff than fd - was using old
				// etemp - unsure which is proper
				// - MEMORY ISSUE MARKER -
				//test ebx, ebx //surreal old
				and ebx, ebx //surreal new
...
#ifdef _XBOX
	//surreal
	sprintf(filenameToReturn, "%s%08x\\%08X-%08X-%02X.%s", g_szPathSaves, currentromoptions.crc1, currentromoptions.crc1, currentromoptions.crc2, currentromoptions.countrycode, Ext);
...
void FileIO_ImportPJ64State(const char *filename)
{
	...
	TRACE1("Import PJ64 state: %s", filename);
	...
	// changed by oDD?
	{
		...
		fread(curHeader, sizeof(uint8), 0x40, temporaryFile);
		fclose(temporaryFile);

		//if(memcmp(header, gMS_ROM_Image, 0x40) != 0)
		if(memcmp(header, curHeader, 0x40) != 0)
		{
...
void FileIO_ExportPJ64State(const char *filename)
{
	...
	//memcpy(header, gMS_ROM_Image, 0x40);
	{
		...
		// read in the rom header
		fseek(temporaryFile, 0, SEEK_SET);
		fread(header, sizeof(uint8), 0x40, temporaryFile);
		fclose(temporaryFile);
	}
