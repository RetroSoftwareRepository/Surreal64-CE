...
void CDaedalusRender::LoadFakeFrameBuffer(void)
{
	...
	gti.HeightToCreate		= g_FakeFrameBufferInfo.createdHeight;
	gti.TLutFmt	= G_TT_RGBA16;	//RGBA16
...
void CDaedalusRender::LoadSprite2D(Sprite2DInfo &info, DWORD ucode)
{
	...
	{
		gti.WidthToCreate	= info.spritePtr->SubImageWidth/info.scaleX;
		gti.HeightToCreate	= info.spritePtr->SubImageHeight/info.scaleY;
		gti.LeftToLoad		= info.spritePtr->SourceImageOffsetS/info.scaleX;
		gti.TopToLoad		= info.spritePtr->SourceImageOffsetT/info.scaleY;
		gti.Pitch	= info.spritePtr->Stride << gti.Size >> 1;
		gti.Pitch	*= info.scaleY;
	}
	else
	{
