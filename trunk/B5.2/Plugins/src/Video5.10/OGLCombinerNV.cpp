...
//========================================================================
COGLColorCombinerNvidia::COGLColorCombinerNvidia(CDaedalusRender *pRender) :
	COGLExtColorCombiner(pRender)
{
...
bool COGLColorCombinerNvidia::Initialize(void)
{
	...
	if( COGLExtColorCombiner::Initialize() )
	{
...
{
	if( !m_bNVSupported )	{COGLExtColorCombiner::InitCombinerCycle12();	return;}
...
{
	StartProfiler(PROFILE_COMBINER_PARSER);
	int stagesForRGB=0;
	...
	*/
	...
	StopProfiler(PROFILE_COMBINER_PARSER);
}
...
void COGLColorCombinerNvidia::GenerateNVRegisterCombinerSettingConstants(int index)
{
	...
	BYTE consts[2] = {info.constant0,info.constant1};
...
{
	if( index < 0 || index >= m_vCompiledSettings.size() )
	{
		TRACE0("NV Register combiner, vector index out of range");
		return;
	}

	StartProfiler(PROFILE_COMBINER_GENERATOR);
	NVRegisterCombinerSettingType &info = m_vCompiledSettings[index];
	...
	StopProfiler(PROFILE_COMBINER_GENERATOR);
}

GLenum COGLColorCombinerNvidia::ConstMap(BYTE c)
{
	switch(c)
	{
	case MUX_0:
...
void Set1Variable(GLenum variable, BYTE val, NVCombinerInputType &record, const NVRegisterCombinerParserType &result, bool forRGB=true)
{
...
{
	glDisable(GL_REGISTER_COMBINERS_NV);
	COGLExtColorCombiner::DisableCombiner();
}

void COGLColorCombinerNvidia::InitCombinerCycleCopy(void)
{
	glDisable(GL_REGISTER_COMBINERS_NV);
	COGLExtColorCombiner::InitCombinerCycleCopy();
}

void COGLColorCombinerNvidia::InitCombinerCycleFill(void)
{
	glDisable(GL_REGISTER_COMBINERS_NV);
	COGLExtColorCombiner::InitCombinerCycleFill();
}

void COGLColorCombinerNvidia::InitCombinerBlenderForSimpleTextureDraw(DWORD tile)
{
	glDisable(GL_REGISTER_COMBINERS_NV);
...
char* FormatStrForFinalStage(BYTE val, char* buf)
{
