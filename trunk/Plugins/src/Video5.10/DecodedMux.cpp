...
static const BYTE sc_Mux32[32] = 
{
	...
	MUX_ENV|MUX_ALPHAREPLICATE, MUX_LODFRAC, MUX_PRIMLODFRAC, MUX_UNK,		// Actually k5
...
void DecodedMux::Decode(DWORD dwMux0, DWORD dwMux1)
{
	m_dwMux0 = dwMux0;
	m_dwMux1 = dwMux1;

	aRGB0  = BYTE((dwMux0>>20)&0x0F);	// c1 c1		// a0
	bRGB0  = BYTE((dwMux1>>28)&0x0F);	// c1 c2		// b0
	cRGB0  = BYTE((dwMux0>>15)&0x1F);	// c1 c3		// c0
	dRGB0  = BYTE((dwMux1>>15)&0x07);	// c1 c4		// d0
	
	aA0    = BYTE((dwMux0>>12)&0x07);	// c1 a1		// Aa0
	bA0    = BYTE((dwMux1>>12)&0x07);	// c1 a2		// Ab0
	cA0    = BYTE((dwMux0>>9 )&0x07);	// c1 a3		// Ac0
	dA0    = BYTE((dwMux1>>9 )&0x07);	// c1 a4		// Ad0
	
	aRGB1  = BYTE((dwMux0>>5 )&0x0F);	// c2 c1		// a1
	bRGB1  = BYTE((dwMux1>>24)&0x0F);	// c2 c2		// b1
	cRGB1  = BYTE((dwMux0    )&0x1F);	// c2 c3		// c1
	dRGB1  = BYTE((dwMux1>>6 )&0x07);	// c2 c4		// d1
	
	aA1    = BYTE((dwMux1>>21)&0x07);	// c2 a1		// Aa1
	bA1    = BYTE((dwMux1>>3 )&0x07);	// c2 a2		// Ab1
	cA1    = BYTE((dwMux1>>18)&0x07);	// c2 a3		// Ac1
	dA1    = BYTE((dwMux1    )&0x07);	// c2 a4		// Ad1
...
int DecodedMux::Count(BYTE val, int cycle, BYTE mask)
{
	BYTE* pmux = m_bytes;
	int count=0;
	int start=0;
...
bool DecodedMux::isUsed(BYTE val, BYTE mask)
{
	BYTE* pmux = m_bytes;
	bool isUsed = false;
	for( int i=0; i<16; i++ )
...
bool DecodedMux::isUsedInAlphaChannel(BYTE val, BYTE mask)
{
	BYTE* pmux = m_bytes;
	bool isUsed = false;
	for( int i=0; i<16; i++ )
...
bool DecodedMux::isUsedInColorChannel(BYTE val, BYTE mask)
{
	BYTE* pmux = m_bytes;
	bool isUsed = false;
	for( int i=0; i<16; i++ )
...
bool DecodedMux::isUsedInCycle(BYTE val, int cycle, CombineChannel channel, BYTE mask)
{
	cycle *=2;
...
bool DecodedMux::isUsedInCycle(BYTE val, int cycle, BYTE mask)
{
...
CombinerFormatType DecodedMux::GetCombinerFormatType(DWORD cycle)
{
	//Analyze the formula
	/*
	C=0					= D
...
void DecodedMux::Reformat(void)
{
	...
	if( (splitType[0] == CM_FMT_TYPE_D && splitType[2]!= CM_FMT_TYPE_NOT_USED ) ||	//Cycle 1 Color
		(isUsedInCycle(MUX_COMBINED,1,COLOR_CHANNEL) == false && splitType[2]!= CM_FMT_TYPE_NOT_USED) )
	{
	...
	if( (splitType[1] == CM_FMT_TYPE_D && splitType[3]!= CM_FMT_TYPE_NOT_USED ) ||	//Cycle 2 Alpha
		(isUsedInCycle(MUX_COMBINED,1,ALPHA_CHANNEL) == false && splitType[3]!= CM_FMT_TYPE_NOT_USED) )
	{
		//Replace cycle 1 color with cycle 2 alpha because we have already replace cycle2's cmb 
		aA0 = aA1;
		bA0 = bA1;
		cA0 = cA1;
		dA0 = dA1;
...
char* DecodedMux::FormatStr(BYTE val, char *buf)
{
...
void DecodedMux::Display(bool simplified)
{
...
void DecodedMux::ReplaceVal(BYTE val1, BYTE val2, int cycle, BYTE mask)
{
	int start = 0;
	int end = 16;
...
DWORD DecodedMux::GetCycle(int cycle, CombineChannel channel)
{
	DWORD* pmux = m_dWords;
...
DWORD DecodedMux::GetCycle(int cycle)
{
	...
	if( !isUsedInColorChannel(MUX_SHADE) && (forceToUsed || max(splitType[0], splitType[2]) >= CM_FMT_TYPE_A_MOD_C_ADD_D) )
	{
		...
		if( countEnv+countPrim > 0 )
		{
			...
			if( isUsedInColorChannel(MUX_SHADE|MUX_ALPHAREPLICATE, mask) )
			{
				...
				ReplaceVal((BYTE)m_dwShadeColorChannelFlag, MUX_SHADE, N64Cycle0Alpha);
				ReplaceVal((BYTE)m_dwShadeColorChannelFlag, MUX_SHADE, N64Cycle1Alpha);
...
void DecodedMux::DisplayConstantsWithShade(DWORD flag,CombineChannel channel)
{
