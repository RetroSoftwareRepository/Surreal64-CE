...
BYTE *tmpBuf;
void ** XBOXJumpTable, ** XBOXDelaySlotTable;
BYTE *XBOXRecompCode, *XBOXRecompPos;
DWORD g_dwNormalCompileBufferSize = 0x01000000;
...
int Allocate_Memory ( void ) {	
	...
	(BYTE *)XBOXJumpTable = (BYTE *) VirtualAlloc( NULL, 0x20000000, MEM_RESERVE | MEM_TOP_DOWN, PAGE_READWRITE );
	if( XBOXJumpTable == NULL ) {  
	...
	/* Recomp code */
	XBOXRecompCode=(BYTE *) VirtualAlloc( NULL, LargeCompileBufferSize + 4, MEM_RESERVE|MEM_TOP_DOWN, PAGE_EXECUTE_READWRITE);
	XBOXRecompCode=(BYTE *) VirtualAlloc( XBOXRecompCode,g_dwNormalCompileBufferSize, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
	...
	/* Delay Slot Table */
	(BYTE *)XBOXDelaySlotTable = (BYTE *) VirtualAlloc( NULL, (0x20000000 >> 0xA), MEM_RESERVE | MEM_TOP_DOWN, PAGE_READWRITE );
	if( XBOXDelaySlotTable == NULL ) {  
		DisplayError("Not enough memory for Delay Slot Table!");
		return FALSE;
	}
	if(VirtualAlloc(XBOXDelaySlotTable, (0x400000 >> 0xA), MEM_COMMIT, PAGE_READWRITE)==NULL) {
		DisplayError("Not enough memory for Delay Slot Table!");
		return FALSE;
	}

	if(VirtualAlloc((BYTE *)XBOXDelaySlotTable + (0x04000000 >> 0xA), (0x2000 >> 0xA), MEM_COMMIT, PAGE_READWRITE)==NULL) {
		DisplayError("Not enough memory for Delay Slot Table!");
		return FALSE;
	}
	...
	/* Jump Table */
	if(VirtualAlloc(XBOXJumpTable, 0x00400000, MEM_COMMIT, PAGE_READWRITE)==NULL) {
		DisplayError("Not enough memory for JumpTable!");
		return FALSE;
	}

	if(VirtualAlloc((BYTE *)XBOXJumpTable + 0x04000000, 0x2000, MEM_COMMIT, PAGE_READWRITE)==NULL) {
		DisplayError("Not enough memory for JumpTable!");
		return FALSE;
	}

	if(VirtualAlloc((BYTE *)XBOXJumpTable + 0x1FC00000, 0x7C0, MEM_COMMIT, PAGE_READWRITE)==NULL) {
		DisplayError("Not enough memory for JumpTable!");
		return FALSE;
	}
	...
	case 0x04400000: 
		...
		case 0x04400000: 
			if (ViStatusChanged != NULL) {
				...
				*((BYTE *)(Jump))=(BYTE)(XBOXRecompPos - Jump - 1);
			}
			break;
...
int r4300i_CPU_MemoryFilter( DWORD dwExptCode, LPEXCEPTION_POINTERS lpEP) {
	...
	if (*TypePos == 0xF3 && *(TypePos + 1) == 0xA5) {
		...
		if ((int)End < RdramSize) {
			for ( count = Start; count < End; count += 0x1000 ) {
				if (N64_Blocks.NoOfRDRamBlocks[(count >> 12)] > 0) {
					...
					memset(XBOXJumpTable + ((count & 0x00FFFFF0) >> 2),0,0x1000);
					*(XBOXDelaySlotTable + count) = NULL;
		...
		if (Start >= 0x04000000 && End < 0x04001000) {
			...
			memset(XBOXJumpTable + (0x04000000 >> 2),0,0x1000);
			*(XBOXDelaySlotTable + (0x04000000 >> 12)) = NULL;
		...
		if (Start >= 0x04001000 && End < 0x04002000) {
			...
			memset(XBOXJumpTable + (0x04001000 >> 2),0,0x1000);
			*(XBOXDelaySlotTable + (0x04001000 >> 12)) = NULL;
	...
	if (PAddr >= 0x10000000 && PAddr < 0x16000000) {
		...
		if ((PAddr - 0x10000000) < RomFileSize) {
			if (SignExtend) {
				...
				(int)*Value = (char)ReadSWORDFromROM(PAddr - 0x10000000);
			} else {
				//*Value = ROM[PAddr - 0x10000000];
				*Value = ReadUBYTEFromROM(PAddr - 0x10000000);
	...
	case 0x00700000:
		if (PAddr < RdramSize) {
			...
			*(BYTE *)(N64MEM+PAddr) = Value;
			...
			memset(XBOXJumpTable+((PAddr & 0xFFFFF000) >> 2),0,0x1000);
			*(XBOXDelaySlotTable + ((PAddr & 0xFFFFF000) >> 12)) = NULL;
		}
		break;
	default:
		return FALSE;
...
void Release_Memory ( void ) {
	...
	VirtualFree( XBOXDelaySlotTable, 0 , MEM_RELEASE);
	VirtualFree( SyncMemory, 0 , MEM_RELEASE);
	VirtualFree( XBOXJumpTable, 0 , MEM_RELEASE);
}

void ResetRecompCode (void) {
	DWORD count, OldProtect;
	XBOXRecompPos = XBOXRecompCode;
	...
	for (count = 0; count < (RdramSize >> 12); count ++ ) {
		if (N64_Blocks.NoOfRDRamBlocks[count] > 0) {
			...
			memset(XBOXJumpTable + (count << 10),0,0x1000);
			*(XBOXDelaySlotTable + count) = NULL;
	...
	if (N64_Blocks.NoOfPifRomBlocks > 0) {
		...
		memset(XBOXJumpTable + (0x1FC00000 >> 2),0,0x1000);
	}
}
