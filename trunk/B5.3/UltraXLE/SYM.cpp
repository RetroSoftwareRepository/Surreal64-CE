...
int sym_add(int addr,char *text0,int patch)
{
    ...
    text=(char *)malloc(strlen(text0)+1);
...
{
    int i,j,k,cl,end,x0,x,y,found,classz;
    ...
    for(i=end-4;i>=base;i-=4)
    {
        ...
        { // some matches found
            ...
            for(j=0;j<total;j++)
            {
                ...
                {
                    x=oscall[j][8];
                    routinecrc2(i,0,(dword *)&x,(dword *)&y);
