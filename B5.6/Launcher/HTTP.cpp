...
extern DWORD GetHostAddress(const char *host);
      ...
      for (;;)
      {
        ...
        if (!*p)
        {
          //CLog::Log(LOGERROR, "Invalid reply from server");
			OutputDebugString("Invalid reply from server\n");  
			Close();
          return false;
        }
      }
...
bool CHTTP::Head(string& strURL)
{
  //CLog::Log(LOGINFO, "Get Header URL: %s", strURL.c_str());
	OutputDebugString("Getting Header URL\n");
...
bool CHTTP::Connect()
{
  ...
  else
  {
    ...
    if (service.sin_addr.s_addr == INADDR_NONE)
    {
      ...
      service.sin_addr.s_addr = GetHostAddress(m_strHostName.c_str());
	  if (service.sin_addr.s_addr == INADDR_NONE || strIpAddress == "")
      {
        ...
        OutputDebugString("ERROR: Problem accessing the DNS. Defaulting to using hard-coded IP address for known Hosts.\n");
		...
		else if (strcmp(m_strHostName.c_str(), "freakdave.xbox-scene.com") == 0)
          service.sin_addr.s_addr = inet_addr("65.98.40.42");
        else
        {
...
int CHTTP::Open(const string& strURL, const char* verb, const char* pData)
{
  ...
  if (!Connect())
  {
    ...
    OutputDebugString("Unable to connect to host\n");
	return 0;
  }
  ...
  strcat(szHTTPHEADER, "\r\n"
         "User-Agent: Surreal64XXX/Beta (Windows; U; Windows NT 5.1; en-US; rv:1.7.7) Gecko/20050414 Firefox/1.0.3\r\n");
  strcat(szHTTPHEADER, "From: Surreal64XXX@xbox-scene.com\r\n");
  if (m_strCookie.size())
  {
...
//      CLog::Log(LOGERROR, "Invalid reply from server");
  ...
  else if (status < 400)
  {
    ...
    if (!CanHandle)
    {
      //CLog::Log(LOGERROR, "Server returned: %d %s", status, strReason.c_str());
		OutputDebugString("Server returned unhandable status\n");
  ...
  {
    //CLog::Log(LOGERROR, "Server returned: %d %s", status, strReason.c_str());
    OutputDebugString("Server returned error\n");
	 Close();
    return status; // error
  }
}
