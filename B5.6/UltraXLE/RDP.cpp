...
typedef struct
{
    ...
    int       txtload[MAXLOAD];
    HTexture   txt[MAXTXT]; // 0 not used
    Tile      tile[8];
...
void rdp_screenshot(char *name0)
{
    ...
    if(!rst.opened)
    {
        xception("graphics not active, cannot take screenshot");
        return;
    }
...
{ // convert from texmem to 32 bit RGBA
    ...
    if(rl2>512*4*2)
    {
        ...
        xception("rdp: invalid texture memory width");
        return;
    }
...
void txt_loaddata(HTexture *txt,Tile *t)
{
    int sx,sy;
    int limit;
    int hasalpha;
...
//    if(rst.s_txtfilt==0) flags|=X_NOBILIN;
    ...
    //if(txt->xhandle<0) xception("too many textures\n");
...
void txt_setscales(int tile)
{
    ...
    //HTexture *txt=rst.txt+t->texture;
    float    xd,yd,xdm,ydm;
...
void flushprims(void)
{
    ...
    if(rst.s_alphatst==4 || (rst.s_alphatst==1))// && rst.colf[C_BLEND][3]>0.1))
    {
		...
		g_pDevice->SetRenderState(D3DRS_ALPHAREF, DWORD(0.25*255.0));
    }
    else
    {
        if(COM.blend1[0]==X_ALPHA)
		{
			g_pDevice->SetRenderState(D3DRS_ALPHATESTENABLE, TRUE);
			g_pDevice->SetRenderState(D3DRS_ALPHAREF, DWORD(0.05*255.0));
		}
        else
		{
			g_pDevice->SetRenderState(D3DRS_ALPHATESTENABLE, FALSE);
			g_pDevice->SetRenderState(D3DRS_ALPHAREF, DWORD(1.00*255.0));
		}
    }
    ...
    { // draw
        for(i=0;i<COM.passes;i++)
        {
			...
			g_pDevice->SetRenderState(D3DRS_TEXTUREFACTOR, D3DCOLOR_RGBA(int(c[0]*255.0f),int(c[1]*255.0f),int(c[2]*255.0f),int(c[3]*255.0f)));
			...
			{
				g_pDevice->SetRenderState(D3DRS_SRCBLEND, g_srcBlendMap[COM.blend1[i] - X_ZERO]);
				g_pDevice->SetRenderState(D3DRS_DESTBLEND, g_destBlendMap[COM.blend2[i] - X_ZERO]);
			}
            ...
            {
				g_pDevice->SetTextureStageState(0, D3DTSS_COLOROP,   g_combineOpMap[COM.com[i][0]-X_WHITE]);
				g_pDevice->SetTextureStageState(0, D3DTSS_COLORARG1, g_combineArg1Map[COM.com[i][0]-X_WHITE]);
				g_pDevice->SetTextureStageState(0, D3DTSS_COLORARG2, g_combineArg2Map[COM.com[i][0]-X_WHITE]);

				g_pDevice->SetTextureStageState(0, D3DTSS_ALPHAOP,   g_combineOpMap[COM.com[i][1]-X_WHITE]);
				g_pDevice->SetTextureStageState(0, D3DTSS_ALPHAARG1, g_combineArg1Map[COM.com[i][1]-X_WHITE]);
				g_pDevice->SetTextureStageState(0, D3DTSS_ALPHAARG2, g_combineArg2Map[COM.com[i][1]-X_WHITE]);

				if (COM.com[i][0] == X_TEXTUREENVC)
				{
					g_pDevice->SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_LERP);
					g_pDevice->SetTextureStageState(0, D3DTSS_COLORARG0, D3DTA_TEXTURE);
				...
				if (COM.com[i][1] == X_TEXTUREENVC)
				{
					g_pDevice->SetTextureStageState(0, D3DTSS_ALPHAOP, D3DTOP_LERP);
					g_pDevice->SetTextureStageState(0, D3DTSS_ALPHAARG0, D3DTA_TEXTURE);
					g_pDevice->SetTextureStageState(0, D3DTSS_ALPHAARG1, D3DTA_DIFFUSE);
					g_pDevice->SetTextureStageState(0, D3DTSS_ALPHAARG2, D3DTA_TFACTOR);
				}

				if (COM.com[i][0] == X_TEXTUREENVCR)
				{
					g_pDevice->SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_LERP);
					g_pDevice->SetTextureStageState(0, D3DTSS_COLORARG0, D3DTA_TEXTURE);
					g_pDevice->SetTextureStageState(0, D3DTSS_COLORARG1, D3DTA_DIFFUSE);
            ...
            if(COM.forceatst[i])
			{
				...
				switch(COM.forceatst[i])
				{
					...
					case  2: 
						...
						break;
					case  3: 
						g_pDevice->SetRenderState(D3DRS_ALPHAREF, DWORD(0.50*255.0));
						break;
					case  4: 
						g_pDevice->SetRenderState(D3DRS_ALPHAREF, DWORD(0.75*255.0));
						break;
					case  9:
						{
							static int cnt=0;
							...
							case 1: 
								...
								break;
							case 2: 
								g_pDevice->SetRenderState(D3DRS_ALPHAREF, DWORD(0.15*255.0));
								break;
							case 3: 
								g_pDevice->SetRenderState(D3DRS_ALPHAREF, DWORD(0.35*255.0));
								break;
							}
						}
						break;
            ...
            {
				g_pDevice->SetRenderState(D3DRS_ZWRITEENABLE, (COM.forcezupd[i] == X_ENABLE));
				g_pDevice->SetRenderState(D3DRS_ZENABLE, (COM.forcezcmp[i] == X_ENABLE));
            }
            else if(i==0)
            {
...
void rdp_tri(int *vind)
{
    ...
    {
        xception("dlist: vx/pr-tab corrupted!\n");
        return;
    }
...
static void opendisplay(void)
{
	...
	/*if (g_defaultTrueTypeFont)
	{
		g_defaultTrueTypeFont->Release();
	}

	XFONT_OpenDefaultFont(&g_defaultTrueTypeFont);*/

	// alpha blending
	g_srcBlendMap[X_ZERO-X_ZERO]				= D3DBLEND_ZERO;
	g_srcBlendMap[X_ONE-X_ZERO]					= D3DBLEND_ONE;
	g_srcBlendMap[X_OTHER-X_ZERO]				= D3DBLEND_DESTCOLOR;
	g_srcBlendMap[X_ALPHA-X_ZERO]				= D3DBLEND_SRCALPHA;
	g_srcBlendMap[X_OTHERALPHA-X_ZERO]			= D3DBLEND_DESTALPHA;
	g_srcBlendMap[X_INVOTHER-X_ZERO]			= D3DBLEND_INVDESTCOLOR;
	g_srcBlendMap[X_INVALPHA-X_ZERO]			= D3DBLEND_INVSRCALPHA;
	g_srcBlendMap[X_INVOTHERALPHA-X_ZERO]		= D3DBLEND_INVDESTALPHA;

	g_destBlendMap[X_ZERO-X_ZERO]				= D3DBLEND_ZERO;
	g_destBlendMap[X_ONE-X_ZERO]				= D3DBLEND_ONE;
	g_destBlendMap[X_OTHER-X_ZERO]				= D3DBLEND_SRCCOLOR;
	g_destBlendMap[X_ALPHA-X_ZERO]				= D3DBLEND_DESTALPHA;
	g_destBlendMap[X_OTHERALPHA-X_ZERO]			= D3DBLEND_SRCALPHA;
	g_destBlendMap[X_INVOTHER-X_ZERO]			= D3DBLEND_INVSRCCOLOR;
	g_destBlendMap[X_INVALPHA-X_ZERO]			= D3DBLEND_INVDESTALPHA;
	g_destBlendMap[X_INVOTHERALPHA-X_ZERO]		= D3DBLEND_INVSRCALPHA;

	// texture combines
	g_combineOpMap[X_COLOR-X_WHITE]				= D3DTOP_SELECTARG1;
	g_combineArg1Map[X_COLOR-X_WHITE]			= D3DTA_DIFFUSE;
	g_combineArg2Map[X_COLOR-X_WHITE]			= D3DTA_DIFFUSE;

	g_combineOpMap[X_TEXTURE-X_WHITE]			= D3DTOP_SELECTARG1;
	g_combineArg1Map[X_TEXTURE-X_WHITE]			= D3DTA_TEXTURE;
	g_combineArg2Map[X_TEXTURE-X_WHITE]			= D3DTA_TEXTURE;

	g_combineOpMap[X_ADD-X_WHITE]				= D3DTOP_ADD;
	g_combineArg1Map[X_ADD-X_WHITE]				= D3DTA_DIFFUSE;
	g_combineArg2Map[X_ADD-X_WHITE]				= D3DTA_TEXTURE;

	g_combineOpMap[X_MUL-X_WHITE]				= D3DTOP_MODULATE;
	g_combineArg1Map[X_MUL-X_WHITE]				= D3DTA_DIFFUSE;
	g_combineArg2Map[X_MUL-X_WHITE]				= D3DTA_TEXTURE;

	g_combineOpMap[X_TEXTUREENVCR-X_WHITE]		= D3DTOP_BLENDFACTORALPHA;
	g_combineArg1Map[X_TEXTUREENVCR-X_WHITE]	= D3DTA_DIFFUSE;
	g_combineArg2Map[X_TEXTUREENVCR-X_WHITE]	= D3DTA_TEXTURE;

	g_combineOpMap[X_TEXTUREBLEND-X_WHITE]		= D3DTOP_BLENDTEXTUREALPHA;
	g_combineArg1Map[X_TEXTUREBLEND-X_WHITE]	= D3DTA_TEXTURE;
	g_combineArg2Map[X_TEXTUREBLEND-X_WHITE]	= D3DTA_DIFFUSE;

	g_combineOpMap[X_SUB-X_WHITE]				= D3DTOP_SUBTRACT;
	g_combineArg1Map[X_SUB-X_WHITE]				= D3DTA_DIFFUSE;
	g_combineArg2Map[X_SUB-X_WHITE]				= D3DTA_TEXTURE;

	g_combineOpMap[X_TEXTUREENVC-X_WHITE]		= D3DTOP_BLENDFACTORALPHA;
	g_combineArg1Map[X_TEXTUREENVC-X_WHITE]		= D3DTA_DIFFUSE;
	g_combineArg2Map[X_TEXTUREENVC-X_WHITE]		= D3DTA_TEXTURE;

	/*g_srcBlendMap[X_ZERO]			= D3DBLEND_ZERO;
	g_srcBlendMap[X_ONE]				= D3DBLEND_ONE;
	g_srcBlendMap[X_OTHER]			= D3DBLEND_DESTCOLOR;
	g_srcBlendMap[X_ALPHA]			= D3DBLEND_SRCALPHA;
	g_srcBlendMap[X_OTHERALPHA]		= D3DBLEND_DESTALPHA;
	g_srcBlendMap[X_INVOTHER]		= D3DBLEND_INVDESTCOLOR;
	g_srcBlendMap[X_INVALPHA]		= D3DBLEND_INVSRCALPHA;
	g_srcBlendMap[X_INVOTHERALPHA]	= D3DBLEND_INVDESTALPHA;

	g_destBlendMap[X_ZERO]			= D3DBLEND_ZERO;
	g_destBlendMap[X_ONE]				= D3DBLEND_ONE;
	g_destBlendMap[X_OTHER]			= D3DBLEND_DESTCOLOR;
	g_destBlendMap[X_ALPHA]			= D3DBLEND_SRCALPHA;
	g_destBlendMap[X_OTHERALPHA]		= D3DBLEND_DESTALPHA;
	g_destBlendMap[X_INVOTHER]		= D3DBLEND_INVDESTCOLOR;
	g_destBlendMap[X_INVALPHA]		= D3DBLEND_INVSRCALPHA;
	g_destBlendMap[X_INVOTHERALPHA]	= D3DBLEND_INVDESTALPHA;*/
...
void rdp_framestart(void)
{
	...
	g_pDevice->GetScissors(&count, &ex, &origScis);
	g_pDevice->SetScissors(1, FALSE, &newScis);*/
	...
	//freakdave
	g_pDevice->SetFlickerFilter(FlickerFilter);
	g_pDevice->SetSoftDisplayFilter(SoftDisplayFilter);

	//g_pDevice->SetScissors(count, ex, &origScis);
...
extern BOOL _INPUT_UpdatePaks();//added by freakdave
...
void rdp_frameend(void)
{
	...
	menuWait = _INPUT_IsIngameMenuWaiting();
