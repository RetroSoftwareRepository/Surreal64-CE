...
int cart_load(char *fname)
{
    ...
    cart.data=(byte *)malloc(cart.size);
...
//    print("mapping handle=%08X\n",cart.maphandle);
    ...
    cart.mapped=1;
    cart.data=(byte *)MapViewOfFile((HANDLE)cart.maphandle,
                      FILE_MAP_READ,
                      0,0,
                      cart.size);
...
void cart_open(char *fname,int memmap)
{
    ...
    if(flip<0)
    {
        if(flip==-1) xception("File '%s' could not be opened.",fname);
        else         xception("File '%s' format unrecognized.",fname);
    ...
    /*if(flip==0x0123 && memmap)
    {
        if(cart_map(fname))
        {
            xception("Error %i mapping file.",GetLastError());
            cart_dummy();
            return;
        }
    }
	...
	// get the crc1 and crc2 of the rom
	cart.crc1 = *((dword *)(cart.data + 0x10));
    ...
    //if(cart.mapped) 
	//	print("ROM image '%s' mapped (%iMB): "YELLOW"%s\n",fname,cart.size/1024/1024,cart.title);
    //else 
	//	print("ROM image '%s' loaded (%iMB): "YELLOW"%s\n",fname,cart.size/1024/1024,cart.title);
}
