...
char		*emulator_type_names[] = { "Default", "Dyna-Compiler", "Interpreter" }; // Ez0n3 - for trace debug
...
double		vips_speed_limits[] = { 500.0f, 500.0f, 59.94f, 50.0f, 59.94f, 10.0f, 15.0f, 20.0f, 25.0f,
									30.0f, 40.0f, 70.0f, 80.0f, 90.0f, 100.0f, 120.0f, 150.0f, 180.0f, 210.0f}; // needed?
...
//surreal - expects USE_ROM_PAGING
 ...
 */
//surreal
int GenerateCurrentRomOptions(void) // void
{
	// surreal
	...
	currentromoptions.FPU_Hack					= defaultoptions.FPU_Hack; //surreal
...
/*#ifdef _DEBUG
	...
	while (TRUE)
	{
		...
		//if(feof(iniFile) != 0) return FALSE; // Ez0n3 - diff than fd - left uncommented

		// Ez0n3 - diff than fd - missing
		if(strlen(line) > 23)
		{
			if(line[0] == '[' && line[23] == ']')
			{
				...
				&& countrycode == currentromoptions.countrycode)
				{
					...
					OutputDebugStringA(" Matching ini entry found!\n");
				}
		...
		else if(strncmp(line, "Use Register Caching=", 21) == 0 && foundMatchingEntry)
		{
			...
			if( currentromoptions.Use_Register_Caching == 0 )
				currentromoptions.Use_Register_Caching = defaultoptions.Use_Register_Caching;
		}
		else if(strncmp(line, "Counter Factor=", 15) == 0 && foundMatchingEntry)
		{
			...
			if( currentromoptions.Counter_Factor == 0 )
				currentromoptions.Counter_Factor = defaultoptions.Counter_Factor;
		}
		else if(strncmp(line, "FPU Hack=", 9) == 0 && foundMatchingEntry)
		{
			...
			if( currentromoptions.FPU_Hack == 0 )
				currentromoptions.FPU_Hack = defaultoptions.FPU_Hack;
		}
		else if(strncmp(line, "DMA=", 4) == 0 && foundMatchingEntry)
		{
			currentromoptions.timing_Control = atoi(line + 4);
			//if(currentromoptions.timing_Control > DELAY_DMA_SI_AI) currentromoptions.timing_Control = 0;
			if( currentromoptions.timing_Control == 0 )
				currentromoptions.timing_Control = defaultoptions.timing_Control;
		}
		else if(strncmp(line, "Link 4KB Blocks=", 16) == 0 && foundMatchingEntry)
		{
			...
			if( currentromoptions.Link_4KB_Blocks == 0 )
				currentromoptions.Link_4KB_Blocks = defaultoptions.Link_4KB_Blocks;
		}
		else if(strncmp(line, "Advanced Block Analysis=", 24) == 0 && foundMatchingEntry)
		{
			...
			if( currentromoptions.Advanced_Block_Analysis == 0 )
				currentromoptions.Advanced_Block_Analysis = defaultoptions.Advanced_Block_Analysis;
		}
		else if(strncmp(line, "Assume 32bit=", 13) == 0 && foundMatchingEntry)
		{
			...
			if( currentromoptions.Assume_32bit == 0 )
				currentromoptions.Assume_32bit = defaultoptions.Assume_32bit;
		}
		else if(strncmp(line, "HLE=", 4) == 0 && foundMatchingEntry)
		{
			...
			if( currentromoptions.Use_HLE == 0 )
				currentromoptions.Use_HLE = defaultoptions.Use_HLE;
		}
		else if(strncmp(line, "RSP_RDP_Timing=", 15) == 0 && foundMatchingEntry)
		{
			currentromoptions.RSP_RDP_Timing = atoi(line + 15);
			//if(currentromoptions.RSP_RDP_Timing > 2) currentromoptions.RSP_RDP_Timing = 0;
			if( currentromoptions.RSP_RDP_Timing == 0 )
				currentromoptions.RSP_RDP_Timing = defaultoptions.RSP_RDP_Timing;
		}
		else if(strncmp(line, "CFB_RW=", 7) == 0 && foundMatchingEntry)
		{
			currentromoptions.frame_buffer_rw = atoi(line + 7);
			//if(currentromoptions.frame_buffer_rw > 2) currentromoptions.frame_buffer_rw = 0;
		...
		else
		{
			...
			currentromoptions.Code_Check		!= CODE_CHECK_NONE &&
...
#ifdef _DEBUG
				DisplayError("Matching rom ini settings found!"); // Ez0n3 - notify if ini settings
#endif
...
#ifdef _DEBUG
	DisplayError("Matching rom ini settings not found!"); // Ez0n3 - notify if not ini settings
#endif
...
{
#ifdef USE_ROM_PAGING // temp fix till ini is working
	SetDefaultOptions();
#endif
...
void chopm(char *str)
{
	...
	*/
	...
	//if (i == 0) return; //surreal old - needed?
