...
//missing from 1.1 source? setting up dummies to satisfy dependencies
int DebuggerOpcodeTraceEnabled = 0;
BOOL DebuggerActive = FALSE;
void HandleBreakpoint(unsigned __int32 Instruction) {}
void ProcessDebugMenuCommand(WPARAM wParam) {}
BOOL DebuggerBreakPointActive = FALSE;
void OpenDebugger(void) {}
void RefreshDebugger(void) {}
unsigned _int32 BreakAddress = -1;
unsigned __int32 NextClearCode = 250;
int OpCount = 0;
void REGISTRY_ReadDebug() {}
int REGISTRY_WriteDebug() { return -1; }
...
//not needed on xbox, the launcher does this
// just includeing to convert roms the same way the launcher does before it starts the emu
#ifdef USE_ROM_PAGING
#define BUFFER_SIZE 0x400000 // -> (4096 / 4KB pagesize) = 1MB
typedef unsigned __int64    qword;
extern BOOL ByteSwap2(uint32 Size, uint8 *Image, uint8 bswaptype);
...
DWORD PrepareRomThread(char *szRomFile)
{
	...
	ofstream destRom;
	destRom.open("C:\\TemporaryRom.dat", ios_base::out | ios_base::binary);
	if (!destRom.is_open())
	{
		goto _FAIL;
	}

	{
		...
		// open the rom
		fp.open(szRomFile, ios_base::in | ios_base::binary);
		if (!fp.is_open())
		{
			goto _FAIL;
		}
		...
		if (!ByteSwap2(BUFFER_SIZE, pBuffer, byBswapType))
		{
			fp.close(); // not a rom file
			goto _FAIL;
		}
		...
		for(qword i = BUFFER_SIZE; i < qwFileSize; i += BUFFER_SIZE)
		{
			...
			// byte swap the meg
			ByteSwap2(BUFFER_SIZE, pBuffer, byBswapType);
...
int APIENTRY aWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int nCmdShow)
{
	...
	//convert normal rom into a temp rom
	/*{
		char szTmpRom[] = "C:\\Super Mario 64 (U) [!].z64";
		
		if (PathFileExists(szTmpRom))
			PrepareRomThread(szTmpRom);
			
		OutputDebugString("Need to restart after converting the rom?\n");
		Sleep(300);
	}*/
	
	// make sure there's a temp rom
	if (PathFileExists("C:\\TemporaryRom.dat")) {
		strcpy(g_temporaryRomPath, "C:\\TemporaryRom.dat");
	} else {
		DisplayError("C:\\TemporaryRom.dat File Not Found!\n");
	}

	g_dwRecompCodeSize = 8 * 1024 * 1024;
	
	g_iPagingMethod = _PagingXXX; //_PagingS10;
...
#ifdef USE_ROM_PAGING //_XBOX
	{
		FILE *tmpFile = fopen(g_temporaryRomPath, "rb"); // load same file every time
