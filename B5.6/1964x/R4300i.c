...
/*
 ...
 =======================================================================================================================
 */
__forceinline void SPECIAL_instr(uint32 Instruction)
{
...
__forceinline void	Display_Address_Error(uint32 addr, char *opcode);
...
/*
 ...
 =======================================================================================================================
 */
__forceinline _int32 r4300i_lb_faster(uint32 QuerAddr)
{
...
/*
 ...
 =======================================================================================================================
 */
__forceinline void r4300i_do_speedhack(void)
{
...
__forceinline void r4300i_lw(uint32 Instruction)
{
	...
	QUER_ADDR;
	CHECKING_ADDR_ALIGNMENT(QuerAddr, 0x3, "LW", EXC_RADE) 
	
	// added by oDD
	// directs all direct rom reads to the paging function
	if (CheckIfInRom(QuerAddr))
	{
		gHWS_GPR[rt_ft] = ReadSWORDFromROM_EAXOnly(QuerAddr);
	}
	else
	{
		gHWS_GPR[rt_ft] = MEM_READ_SWORD(QuerAddr);
	}
...
__forceinline void r4300i_BootHelper(HardwareState *gHWState, MemoryState *gMemoryState)
{
	...
	// added by oDD
	bootaddr = ReadUWORDFromROM(8) & 0x007FFFFF;
	...
	//memcpy(gMemoryState->RDRAM + bootaddr, gMemoryState->ROM_Image + 0x1000, 0x400000 - bootaddr);
	{
		...
		fseek(tmpFile, 0x1000, SEEK_SET);
		fread(gMemoryState->RDRAM + bootaddr, sizeof(uint8), 0x400000 - bootaddr, tmpFile);
		fclose(tmpFile);
	}
...
__forceinline void r4300i_ResetMemory(MemoryState *gMemoryState)
{
	...
	//memcpy((uint8 *) &gMemoryState->SP_MEM[0], gMemoryState->ROM_Image, 0x1000);
	{
		...
		fread((uint8 *) &gMemoryState->SP_MEM[0], sizeof(uint8), 0x1000, tmpFile);
		fclose(tmpFile);
	}
}
...
/*
 ...
 =======================================================================================================================
 */
__forceinline void SW_AddrError(uint32 QuerAddr, uint32 val)
{
	DISPLAY_ADDRESS_ERROR(QuerAddr, "SW");
}
