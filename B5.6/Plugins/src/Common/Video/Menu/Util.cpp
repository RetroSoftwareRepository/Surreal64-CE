...
__forceinline string Trim(const string &str)
{
	string szStr(str);
	string::size_type i;
...
__forceinline string FixPath(const string &path)
{
	string szPath(path);
...
__forceinline word *StringToWChar(const string &str)
{
...
__forceinline byte GetByteSwapType(byte *header)
{
...
__forceinline bool ByteSwap(dword size, byte *image, byte bswapType)
{
	dword k;
...
__forceinline void SwapRomName(byte *name)
{
	byte c;
