...
protected:
	...
	COGLColorCombiner(CDaedalusRender *pRender);
	~COGLColorCombiner();
...
public:
	...
	void BlendFunc(DWORD srcFunc, DWORD desFunc);
	void Enable();
	void Disable();

protected:
	friend class OGLDeviceBuilder;
	COGLBlender(CDaedalusRender *pRender) : CBlender(pRender), m_pOGLRender((OGLRender*)pRender) {};
	~COGLBlender() {};
