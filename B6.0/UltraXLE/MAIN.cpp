...
void tmpSleep(unsigned int milliseconds)
{
	Sleep(milliseconds);
}
...
void outputhook(char *txt,char *full)
{
    ...
    // Exceptions start with 'xception(address):' and
...
//    xception("main_stop failed (breakout=%i, bailout=%i, exec=%i)!",st.breakout,st.bailout,st.executing);
    //flushdisplay();
...
char *main_command(char *cmd,...)
{
    ...
    if(wasexecuting)
    {
        if(main_stop())
        {
            ...
            cmderrs=3;
            return("xception(00000000): Cannot stop execution");
        }
    }
