...
void mem_init(int ramsize)
{
    ...
    mem.code=(byte *)malloc(mem.codemax);
    ...
    mem.group=(Group *)malloc(sizeof(Group)*mem.groupmax);
    ...
    mem.groupcnt=(int *)malloc(sizeof(int)*mem.groupmax);
...
dword mem_getphysical(dword virt)
{
    byte *raw=(byte *)memdatar(virt);
...
void mem_load(FILE *f1)
{
    ...
    if(!mem.ramsize)
    {
        ...
        if(getc(f1)!=0xfc)
        {
            xception("mem_load fatal error (vm)!");
            return;
        }
    }
    else
    {
        ...
        for(rw=0;rw<2;rw++)
        {
            ...
            for(i=0;i<1048576;)
            {
                ...
                if(n<0)
                {
                    xception("mem_load fatal error (eof)!\n");
                    return;
                }
                fread(&m,1,4,f1);
                /*
    ...
    for(i=0;i<ramsize2;)
    {
        ...
        if(n<0)
        {
            xception("mem_load fatal error (eof1-%06X)!",i);
            return;
        }
        fread(&x,1,4,f1);
        while(n-->0) ram[i++]=x;
        ...
        if(n<0)
        {
            xception("mem_load fatal error (eof2-%06X)!",i);
            return;
        }
        fread(ram+i,n,4,f1);
        i+=n;
    ...
    if(getc(f1)!=0xfc)
    {
        xception("mem_load fatal error (data)!");
    }
}
...
void  mem_readrangeraw(dword addr,int bytes,char *data)
{
    ...
    if((addr&3) || (bytes&3)) xception("mem_readrangeraw: align error (%i bytes at %08X)",bytes,addr);
...
void  mem_writerangeraw(dword addr,int bytes,char *data)
{
    ...
    if((addr&3) || (bytes&3)) xception("mem_writerangeraw: align error (%i bytes at %08X)",bytes,addr);
