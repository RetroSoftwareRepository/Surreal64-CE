...
FUNC_TYPE(void) NAME_DEFINE(GetDllInfo) ( PLUGIN_INFO * PluginInfo )
{
	...
	sprintf(PluginInfo->Name, "%s Debug %s - DX9",project_name, FILE_VERSION);
#else
	sprintf(PluginInfo->Name, "%s Debug %s",project_name, FILE_VERSION);
#endif
#else
	sprintf(PluginInfo->Name, "%s Debug %s - Win98",project_name, FILE_VERSION);
#endif
#else
#if _MSC_VER > 1200
#if DIRECTX_VERSION > 8
...
#endif
#else
	sprintf(PluginInfo->Name, "%s %s - Win98",project_name, FILE_VERSION);
#endif
#endif
...
FUNC_TYPE(void) NAME_DEFINE(DllAbout) ( HWND hParent )
{
	char temp[300];
#if DIRECTX_VERSION == 8
	sprintf(temp,"%s %s (build %s) \nDirectX 8.1+\nOpenGL 1.1-2.0",project_name, FILE_VERSION,BUILD_NUMBER) ;
#else
	sprintf(temp,"%s %s (build %s) \nDirectX 9\nOpenGL 1.1-1.4/ATI/Nvidia TNT/Geforce Extension",project_name, FILE_VERSION,BUILD_NUMBER) ;
#endif
	MsgInfo(temp);
}
...
FUNC_TYPE(void) NAME_DEFINE(CaptureScreen) ( char * Directory )
{
	...
	if( status.bGameIsRunning && status.gDlistCount > 0 )
	{
		...
		for( int i=0; ; i++)
		{
			...
			{
				sprintf(tempname, "%s-%d.jpg", status.screenCaptureFilename, i);
				if( !PathFileExists(tempname) )
				{
					sprintf(tempname, "%s-%d.jpg", status.screenCaptureFilename, i);
					break;
