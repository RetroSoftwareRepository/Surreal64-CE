...
class CDaedalusRender;

extern char* cycleTypeStrs[];

class CColorCombiner
{
	friend class CDaedalusRender;
public:
	virtual ~CColorCombiner() {};
	DaedalusColor GetConstFactor(DWORD colorFlag, DWORD alphaFlag, DWORD defaultColor = 0);
...
protected:
	CColorCombiner(CDaedalusRender *pRender) : 
...
DaedalusColor CalculateConstFactor(DWORD colorOp, DWORD alphaOp, DWORD curCol=0);
