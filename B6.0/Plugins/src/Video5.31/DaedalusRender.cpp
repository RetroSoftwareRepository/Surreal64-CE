...
extern void SetFakeFrameBuffer(void);
...
bool CDaedalusRender::FillRect(LONG nX0, LONG nY0, LONG nX1, LONG nY1, DWORD dwColor)
{
	...
	if( status.bHandleN64TextureBuffer && !status.bDirectWriteIntoRDRAM )	status.bTextureBufferIsDrawn = true;
...
bool CDaedalusRender::Line3D(DWORD dwV0, DWORD dwV1, DWORD dwWidth)
{
	...
	{
		g_FakeFrameBufferInfo.maxUsedHeight = g_FakeFrameBufferInfo.createdHeight;
		if( status.bHandleN64TextureBuffer && !status.bDirectWriteIntoRDRAM )	
		{
			status.bTextureBufferIsDrawn = true;
...
bool CDaedalusRender::TexRect(LONG nX0, LONG nY0, LONG nX1, LONG nY1, float fS0, float fT0, float fScaleS, float fScaleT, bool colorFlag, DWORD diffuseColor)
{
	...
	if( options.enableHackForGames == HACK_FOR_DR_MARIO )
	{
		...
		if( g_CI.dwWidth  == 320 && nX1 == 328 && g_TI.dwFormat == G_IM_FMT_RGBA && g_TI.dwSize == G_IM_SIZ_16b &&
			nX0 == 0 )
		{
	...
	if( status.bHandleN64TextureBuffer && g_FakeFrameBufferInfo.CI_Info.dwSize == G_IM_SIZ_8b )	
	{
		return true;
	}
	...
	if( frameBufferOptions.bIngoreIfCIIsZI && g_CI.dwAddr == g_DI.dwAddr )
	{
		return true;
	}

	if( g_CI.dwAddr == g_DI.dwAddr && frameBufferOptions.bProcessZIDrawing && !status.bHandleN64TextureBuffer )
	{
		status.bHandleN64TextureBuffer = true;
		SetFakeFrameBuffer();	// Set CI=ZI as texture buffer
		status.bTextureBufferIsDrawn = true;
		status.bTextureBufferDrawnByTriangles = true;
	}
...
void CDaedalusRender::UpdateClipRectangle()
{
	if( status.bHandleN64TextureBuffer )
	{
		...
		windowSetting.vpRightW = g_FakeFrameBufferInfo.width;
		windowSetting.vpBottomW = g_FakeFrameBufferInfo.createdHeight;
		windowSetting.vpWidthW = g_FakeFrameBufferInfo.width;
		windowSetting.vpHeightW = g_FakeFrameBufferInfo.createdHeight;
		...
		int halfx = g_FakeFrameBufferInfo.width/2;
		int halfy = g_FakeFrameBufferInfo.createdHeight/2;
		int centerx = halfx;
		int centery = halfy;
