...
extern void _INPUT_SetRumble(int Control, BOOL on);
		...
		if (Controllers[Control].Present == TRUE) {
			...
			switch (Controllers[Control].Plugin) 
			{
				case PLUGIN_RUMBLE_PAK:
				{
					...
					Data[32] = Mempacks_CalulateCrc(Data);
					break;
				}
				case PLUGIN_MEMPAK: 
			...
			switch (Controllers[Control].Plugin) 
			{
				case PLUGIN_RUMBLE_PAK:
				{
					...
					if(dwAddress == 0xC000)
					{
						...
						if(*Data)
						{
							_INPUT_SetRumble(Control, TRUE);
						}
						else
						{
							_INPUT_SetRumble(Control, FALSE);
						}
					}
