...
void InitVirtualDynaMemory(void)
{
	...
	(double *) dyna_RecompCode = (double *) VirtualAlloc(NULL, g_dwRecompCodeSize, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
...
void InitVirtualMemory1(MemoryState *gMemoryState)
{
	...
	if (gMemoryState->dummyReadWrite == NULL)
		OutputDebugString("dummyReadWrite malloc failed!");
	...
	(uint8 *) gMemoryState->RDRAM = (uint8 *) VirtualAlloc
		(
			NULL,
			MEMORY_SIZE_RDRAM + MEMORY_SIZE_EXRDRAM,
			MEM_RESERVE,
...
void ROM_CheckSumMario(uint8 *buf)
{
	...
	if(a3 != rom[0x10 >> 2] || s0 != rom[0x14 >> 2])
	{
		...
		OutputDebugString("Warning, CRC values don't match, fixed");
	}
}
