...
//#include "../rompaging.h"
_s32 g_tempValue = 0; //surreal - added by oDD
#endif
...
#ifdef _XBOX //_XBOX
	SetTranslator(dyna_RecompCode, 0, g_dwRecompCodeSize);
#else
...
------------------------------------------------------------------------------------------------
	...
	MEMORY ISSUE MARKER START
	
	Moving it to the top to make it easy to compare
...
------------------------------------------------------------------*/
			...
			if (CheckIfInRom(QuerAddr))
			{
				...
				MOV_MemoryToReg(xRT->x86reg, ModRM_disp32, (_u32)&g_tempValue);
			}
			else
			{
				//original
