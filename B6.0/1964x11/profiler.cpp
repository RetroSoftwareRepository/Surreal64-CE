...
//surreal
uint32 g_pageFunctionHits	= 0;
uint32 g_memFunctionHits	= 0;
uint32 g_pageHits			= 0;
uint32 g_pageMisses			= 0;
uint32 g_dynaHits			= 0;
#endif
...
void format_profiler_result_msg(char *msg)
{
    ...
    if(totaltimer == 0) totaltimer = 1;
		...
		sprintf
			(
			msg,
			"[cpu-%2.1f%% vid-%2.1f%% aud-%2.1f%% dyn-%2.1f%%][pfh-%u mfh-%u ph-%u pm-%u dh-%u]",
...
#elif !defined(_XBOX) // win32 - no paging
