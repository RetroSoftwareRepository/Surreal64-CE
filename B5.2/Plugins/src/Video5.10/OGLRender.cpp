...
void OGLRender::SetMinFilter(DWORD dwMinFilter)
{
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, dwMinFilter);
}

void OGLRender::SetMagFilter(DWORD dwMagFilter)
{
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, dwMagFilter);
}

void OGLRender::SetTextureFilter(DWORD dwFilter)
{
	if( options.forceTextureFilter == FORCE_DEFAULT_FILTER )
	{
		switch(dwFilter)
		{
			case G_TF_AVERAGE:	//?
			case G_TF_BILERP:
				m_dwMinFilter = m_dwMagFilter = GL_LINEAR;
				break;
			default:
				m_dwMinFilter = m_dwMagFilter = GL_NEAREST;
				break;
		}
	}
	else
