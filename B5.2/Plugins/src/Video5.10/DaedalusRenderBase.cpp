...
void InitVertexTextureConstants()
{
	...
	gRSP.tex0OffsetX = tile0.hilite_sl/tex0.m_fTexWidth;
	gRSP.tex0OffsetY = tile0.hilite_tl/tex0.m_fTexHeight;
	...
	if( CDaedalusRender::g_pRender->IsTexel1Enable() )
	{
		...
		gRSP.tex1OffsetX = tile1.hilite_sl/tex1.m_fTexWidth;
		gRSP.tex1OffsetY = tile1.hilite_tl/tex1.m_fTexHeight;
	}
...
void InitVertex(TLITVERTEX &v, DWORD dwV, DWORD vtxIndex, bool bTexture, bool openGL)
{
	...
	if( options.gamma_correction )
	{
		v.dcDiffuse = GammaCorrection(v.dcDiffuse);
	}
...
void SetNewVertexInfoNoSSE(DWORD dwAddress, DWORD dwV0, DWORD dwNum)
{
	...
	for (i = dwV0; i < dwV0 + dwNum; i++)
	{
		...
		if( g_curRomInfo.bPrimaryDepthHack && gRDP.otherMode.depth_source )
		{
