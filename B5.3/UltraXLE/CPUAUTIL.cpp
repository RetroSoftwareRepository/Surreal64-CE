...
void insertimmopcode(int opcode,int xrs,int imm)
{
    ...
    if(!(opcode&0xf000))
    {
        xception("cpua: insertimmopcode with X_NONIMM opcode\n");
    }
...
void a_clearcodecache(void)
{
    ...
    if(clears>100)
    {
        xception("Over 100 code clears. Code cache size too small.\n");
    }
    */
...
int reg_allocnew(int name)
{
    ...
    if(!name)
    {
        xception("cpuautil: reg_allocnew with r0 at %08X\n",r.pc);
        return(REG_NONE);
    }
    else if(name==0x1f)
    {
    ...
    if(x<0 || x>7 || x==REG_ESP) xception("cpuautil: reg_alloc (name=%i, x=%i, pc=%08X)",name,x,r.pc);
...
void reg_load(int x86)
{
    ...
    else xception("cpuanew: reg_save unrecognized register type");
}
...
void reg_save(int x86)
{
    ...
    {
        xception("cpuautil: reg_save %i at %08X!\n",x86,r.pc);
    }
    if(type==XNAME_MIPS)
    {
...
int freg_find(int name)
{
    ...
    if(fpregnum>8)
    {
        xception("cpuautil: fpregnum %i > 8\n",fpregnum);
    }
    ...
    for(i=0;i<fpregnum;i++)
    {
        ...
        {
            if(j!=REG_NONE) xception("cpuautil: duplicate fpreg\n");
            j=i;
        }
    }
...
void freg_load(int name,int size)
{
    ...
    else xception("cpuautil: illegal freg_load\n");
}
...
void freg_save(int i)
{
    ...
    else xception("cpuautil: illegal freg_save\n");
